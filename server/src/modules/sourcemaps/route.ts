...
export const sourcemapsUploadRoute: FastifyPluginAsync = async (app) => {
  ...
  app.post("/api/sourcemaps/upload", async (request, reply) => {
    if (!isAuthorized(request, app.config.managementAuthToken)) {
...
function isAuthorized(request: FastifyRequest, expected: string): boolean {
  const header = request.headers.authorization;
  const value = Array.isArray(header) ? header[0] : header;
  return verifyManagementBearerToken(value, expected);
}
